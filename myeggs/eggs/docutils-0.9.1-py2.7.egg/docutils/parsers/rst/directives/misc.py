...
class Title(Directive):
    ...
    def run(self):
        global markbook_title
        markbook_title = self.arguments[0]
...
class Date(Directive):
    ...
    def run(self):
        ...
        global markbook_date
        markbook_date = self.content[0].split(" ")[0]
        text = time.strftime(format)
        return [nodes.Text(text)]

class Publish(Directive):
    ...
    def run(self):
        ...
        source_dir = os.path.join(os.path.expanduser('~'), '.marboo/source/blogs/my_blog')
        try:
            dest = os.path.join(source_dir, '%s-%s' % (markbook_date, os.path.basename(src)))
            if value == 'NO':
                os.system("rm %s" % dest)
            elif value == 'YES':
                ...
                f.write('---\nlayout: post\n')
                try:
                    f.write('title: %s\n' % eval('u"%s"' % markbook_title))
                except NameError:
                    pass
                f.write('---\n\n')
                r = open(src, 'r')
                f.write(r.read().replace('\n.. publish::', '\n.. publish:'))
                r.close()
                f.close()
        except Exception, e:
            pass
